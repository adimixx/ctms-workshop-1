...
int VesselManagement::Vessel_VesselDecksDetails(Vessel vessel, bool showDet){
    auto decks = from(db->deck)>>where([&](Deck const &x){return x.VesselID == vessel.ID;})>>orderby_ascending([&](Deck const &x){return x.Level;})>>to_list();
...
void VesselManagement::Deck_Details(Deck b, bool viewIndex) {
    ...
    if (b.PremiumID != 0){
        ...
        cout  << "Premium Type : \t" + (from(db->value_type)>>first_or_default([&](Value_Type const &x){return x.ID == b.PremiumID;})).name << endl;
