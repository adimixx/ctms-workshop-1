...
int VesselManagement::Deck_level(int DeckID) {
    ...
    do {
        ...
        if ((deckList >> where([&](Deck const &a) { return a.Level == level; }) >> count()) != 0) {
...
void VesselManagement::Deck_AddEdit(int DeckID) {
    ...
    do {
        ...
        out += ((deck.PremiumID == NULL) ? "4 - Add Premium" : "");
...
void VesselManagement::Deck_Delete() {
    ...
    do {
        ...
        auto check = from(db->deck)
                >> where([&](Deck const &a) { return a.VesselID == SelectedVesselID && a.Level == InputInt; });
        if ((check >> count()) == 0) {
