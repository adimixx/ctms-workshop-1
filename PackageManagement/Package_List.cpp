...
void PackageManagement::detail_route(vector<Package_Route> route, bool showInd) {
    ...
    for (int j = 0; j < route.size(); ++j) {
        cout << j + 1 << " : "
             << (from(db->route) >> first_or_default([&](Route const &x) { return x.ID == route.at(j).RouteID; })).name<<endl;
    }
}

void PackageManagement::detail_vessel(Vessel vessel, bool showInd) {
    auto vesselDeck = from(db->deck) >> where([&](Deck const &x) { return x.VesselID == vessel.ID; })
                                     >> orderby_ascending([&](Deck const &x) { return x.Level; });
    ...
    cout << "Maximum Capacity : \t" + to_string(vesselDeck >> sum([&](Deck const &x) { return x.MaxPassenger; })) +
            " Passengers\n";
}
